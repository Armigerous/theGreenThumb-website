 ...
 * Uses task-based statistics from materialized view for optimal performance.
 */
...
export const MyGardenClient: React.FC<MyGardenClientProps> = ({
  ...
  const { gardens, statistics } = gardenData;

  // Reason: Use the properly calculated plants needing care count from the database
  // This avoids double-counting plants that have both overdue and urgent tasks
  const plantsNeedingCareCount = statistics.plantsNeedingCare;
  ...
  useEffect(() => {
    ...
    if (gridRef.current) {
      const cards =
        gridRef.current.querySelectorAll<HTMLElement>(".garden-card");
      withReducedMotion(() =>
  ...
  return (
    <>
      ...
      <div className="relative min-h-screen">
        {/* Reason: Show empty state when user has no gardens yet */}
        {gardens.length === 0 ? (
          <EmptyGardenState />
        ) : (
          <div className="max-w-7xl mx-auto py-8 space-y-8">
            ...
            <CareAlertBanner
              plantCount={plantsNeedingCareCount}
              animated={true}
            />
