...
const Text = () => {
	...
	return (
		<motion.div
			className="text-left md:w-1/2 mb-6 md:mb-0 pt-12 sm:pt-16 lg:pt-0 px-4 sm:px-6 space-y-4 sm:space-y-6"
			initial="hidden"
			animate="visible"
			variants={{
				hidden: { opacity: 0 },
		...
		>
			<motion.p
				className="text-medium sm:text-base lg:text-lg mb-4 italic font-paragraph"
			...
			<motion.h1
				className="text-3xl sm:text-4xl md:text-6xl mb-4 font-title-bold leading-tight sm:leading-normal md:leading-relaxed lg:leading-normal"
				initial={{ opacity: 0, y: 20 }}
				animate={{ opacity: 1, y: 0 }}
				transition={{ duration: 0.6 }}
			>
				Your Garden&apos;s{" "}
				<span className="hover:text-primary font-title-bold inline-block relative w-[7ch]">
			...
			<motion.p
				className="mb-4 text-sm sm:text-base md:text-xl max-w-full leading-relaxed font-paragraph"
				variants={fadeInUp}
			>
			...
			<motion.ul
				className="list-none space-y-2 sm:space-y-3 text-sm sm:text-base md:text-lg"
			...
			>
				{[
					...
					<motion.li
						key={index}
						className="flex items-center font-paragraph-semibold"
						variants={fadeInUp}
					>
			...
			>
				<Link href="/tips">
					<Button
						variant={"default"}
						className="flex justify-center items-center mt-6 w-full text-cream-50 text-base sm:text-lg md:text-xl lg:text-2xl py-4 sm:py-6 bg-primary transition-all transform hover:scale-105 focus:ring focus:ring-brand-600 shadow-lg"
